...
import com.tradesk.appCode.profileModule.gallaryModule.*
...
class SubGallaryActivity : BaseActivity(), SingleListCLickListener, ILeadDetailView {
    ...
    private fun callApi() {
        if (intent.hasExtra("additionalimages")){
            ...
            textView6.setText(intent.getStringExtra("title")!! + " Gallary")



            /*For gallary adapter*/
            mSubListUpdatedAdditionalImage = intent.getStringArrayListExtra("imagelist")   as ArrayList<AdditionalImageImageClient>
            val mAdditionalGallaryAdapter by lazy { SubAdditionalGallaryAdapter(this,intent.getStringExtra("title")!!,this,mSubListUpdatedAdditionalImage) }
            GridLayoutManager(this,   2,  RecyclerView.VERTICAL,   false).apply {
                rvGallary.layoutManager = this
            }
    ...
    override fun onSingleListClick(item: Any, position: Int) {
        if (item.equals("Click")){
            if (intent.hasExtra("additionalimages")) {
                ...
                openApp(this@SubGallaryActivity,mListDocuments[position])
            }
            ...
            openAppPermits(this@SubGallaryActivity,mPermits[position].image,mPermits.get(position)._id)
...
//            )
    ...
    fun showImagePop() {
        ...
        mBuilder.findViewById<TextView>(R.id.titleCameraGallary).also {
            ...
            it.setOnClickListener {
                mBuilder.dismiss()
                Pix.start(this@SubGallaryActivity, options)
            }
        }
    ...
    private fun dispatchTakePictureIntent() {
        Intent(MediaStore.ACTION_IMAGE_CAPTURE).also { takePictureIntent ->
            ...
            takePictureIntent.resolveActivity(this@SubGallaryActivity.packageManager)?.also {
                // Create the File where the photo should go
                val photoFile: File? = try {
                    ConstUtils.createImageFile(this@SubGallaryActivity)
                ...
                photoFile?.also {
                    val photoURI: Uri = FileProvider.getUriForFile(
                        this@SubGallaryActivity,
                        "${this@SubGallaryActivity.packageName}.provider",
                        it
                    )
    ...
    private fun saveCaptureImageResults(path: String) = try {
        val file = File(path!!)
        mFile = Compressor(this@SubGallaryActivity)
