...
import com.tradesk.data.entity.LeadsDataImageClient
import com.tradesk.listeners.SingleListCLickListener
import com.socialgalaxyApp.util.extension.loadWallRound
import kotlin.collections.ArrayList

class PermitsAdapterGallary(
    context: Context,
    val listener: SingleListCLickListener,
    var mImages: ArrayList<LeadsDataImageClient>
) : RecyclerView.Adapter<PermitsAdapterGallary.VHolder>() {
    ...
    override fun onBindViewHolder(holder: VHolder, position: Int) {
        holder.apply {
            val ext = MimeTypeMap.getFileExtensionFromUrl(mImages[position].additional_images[0].image)
...
//            ivMenuTwo.isVisible = isDoc
            if (isDoc) {
                tvDocument.text=mImages[position].client_details.name+" - "+mImages[position].additional_images[0].image.substringAfterLast("/").capitalize()
            } else {
                mIvPic.loadWallRound(mImages[position].additional_images[0].image)
            }



            mImageClick.setOnClickListener { listener.onSingleListClick("PermitAdapter", position) }
            mDocClick.setOnClickListener { listener.onSingleListClick("PermitAdapter", position) }
